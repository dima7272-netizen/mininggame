...
function ArenasScreen({ known, commitNumber }: EditorProps) {
  ...
  return <><PageHeading eyebrow="Множители" title="Арены" subtitle="Каждая строка показывает настоящую модель арены из игрового мира и её баланс." /><div className="notice"><strong>Изображения взяты из игры</strong><p>Источник — модели <code>Workspace.Gameplay.POI.AFK</code>. Служебные метки не попадут в игровой JSON.</p></div><article className="panel editor-panel arenas-editor"><div className="table-scroll"><table className="data-table"><thead><tr><th>Арена из игры</th><th>Множитель</th><th>Требуется ребёртов</th><th>Служебная метка</th></tr></thead><tbody>{known.arenas.map((arena, index) => { const image = getEntityIcon('arenas', String(arena.id)); return <tr key={arena.id}><td><div className="arena-model-cell"><GameEntityIcon category="arenas" entityId={String(arena.id)} size="xl" className="arena-image" /><span><strong>Арена {arena.id}</strong><code>{image?.displayName ?? `ID ${arena.id}`}</code></span></div></td><td><ExactInput label={`Множитель арены ${arena.id}`} value={arena.multiplier} onCommit={(value) => commitNumber('Arenas', `$/${index}/multiplier`, value)} /></td><td>{arena.requiredRebirths ? <ExactInput label={`Ребёрты арены ${arena.id}`} value={arena.requiredRebirths} onCommit={(value) => commitNumber('Arenas', `$/${index}/requiredRebirths`, value)} /> : <span className="muted">Нет поля</span>}</td><td><input className="service-input" value={labels[arena.id] ?? 'Обычная / не классифицирована'} onChange={(event) => setLabels((current) => ({ ...current, [arena.id]: event.target.value }))} /></td></tr>; })}</tbody></table></div></article></>;
}
