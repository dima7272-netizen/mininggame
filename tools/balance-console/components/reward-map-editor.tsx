...
function RewardCellEditor({
  ...
  return <article className="panel map-cell-editor">
    ...
    <div className="cell-primary-actions"><button className="button primary" disabled={!canEdit || locked || !validWeight(draft)} onClick={() => onApply(true)}>{weight === null ? 'Добавить и нормализовать' : 'Изменить + нормализовать'}</button><button className="button secondary" disabled={!canEdit || locked || !validWeight(draft)} onClick={() => onApply(false)}>Оставить временную сумму</button>{weight !== null && <button className="button secondary danger-outline" disabled={!canEdit || locked} onClick={onRemove}>Удалить</button>}</div>
    <div className="cell-tools"><button className={locked ? 'active' : ''} disabled={!canEdit} onClick={onToggleLock}>{locked ? '■ Значение заблокировано' : '□ Заблокировать ячейку'}</button><button disabled={!canEdit} onClick={onNormalize}>Нормализовать комнату</button><button disabled={!canEdit || roomIndex <= 1} onClick={onCopyPrevious}>Копировать предыдущую</button><button disabled={!canEdit} onClick={() => onShift(-1)}>← Схема назад</button><button disabled={!canEdit} onClick={() => onShift(1)}>Схема вперёд →</button></div>
