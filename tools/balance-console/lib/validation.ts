...
export function validateConfigs(
  ...
  if (sellOrder.join('\n') !== rewardHierarchyItemIds.join('\n')) {
    issues.push(issue(
      ...
      'Порядок должен повторять игровую иерархию комнат: Обычные → Необычные → Редкие → Эпические → Легендарные → Мифические → Секретные → Божественные → Небесные. Богоподобные исключены из наград и зарезервированы под золотых питомцев.',
      'SellItems',
      '$/items',
    ));
  }
  ...
  if (
    ...
    issues.push(issue(
      ...
      `После ${known.rooms.at(-1)?.index ?? 'последней'}-й комнаты HP может остановиться`,
  ...
  issues.push(issue(
    'observation',
    'formula.connected',
    'Игровые формулы подключены к симулятору',
    'Источник проверен по Roblox place версии 89: урон, награда Strength, аддитивные множители, бонус друзей, деньги, число предметов и таймер кирки.',
  ));
  issues.push(issue(
    'observation',
